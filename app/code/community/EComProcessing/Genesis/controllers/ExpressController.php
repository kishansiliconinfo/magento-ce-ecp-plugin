...
class EComProcessing_Genesis_ExpressController extends Mage_Core_Controller_Front_Action
{
	/**
	...
	public function notifyAction()
	{
		...
		/** @var EComProcessing_Genesis_Model_Standard $standard */
		$standard   = Mage::getModel('ecomprocessing/express');
	...
	/**
	 * When a customer chooses E-ComProcessing Express on
	...
	public function redirectAction()
	{
		...
		$session->setEComProcessingExpressQuoteId($session->getQuoteId());
		$this->getResponse()->setBody(
			$this->getLayout()->createBlock('ecomprocessing/redirect_express')->toHtml()
		);
		$session->unsQuoteId();
		$session->unsEComProcessingExpressRedirectUrl();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$session->setQuoteId($session->getEComProcessingExpressQuoteId(true));
	...
	public function cancelAction()
	{
		...
		if ($session->getLastRealOrderId()) {
			...
			Mage::helper('ecomprocessing')->restoreQuote();
		}
