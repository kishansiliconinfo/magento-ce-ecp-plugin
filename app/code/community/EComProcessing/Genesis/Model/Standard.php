 ...
 * @category
 */
class EComProcessing_Genesis_Model_Standard extends Mage_Payment_Model_Method_Cc
{
    protected $_code = 'ecomprocessing_genesis';
	...
	public function __construct() {
		$environment = intval(Mage::helper('ecomprocessing_genesis')->getConfigVal('genesis_environment')) == 1 ? 'sandbox' : 'production';

		GenesisConf::setToken(
			Mage::helper('ecomprocessing_genesis')->getConfigVal('genesis_token')
		);
		GenesisConf::setUsername(
			Mage::helper('ecomprocessing_genesis')->getConfigVal('genesis_username')
		);
		GenesisConf::setPassword(
			Mage::helper('ecomprocessing_genesis')->getConfigVal('genesis_password')
		);
	...
	public function authorize($payment, $amount)
	{
		try {
			...
			$transaction_id = Mage::helper('ecomprocessing_genesis')->genTransactionId();
			$remote_address = Mage::helper('core/http')->getRemoteAddr(false);

			$usage = Mage::helper('ecomprocessing_genesis')->getItemList($order);
			...
			Mage::throwException(
				Mage::helper('ecomprocessing_genesis')->__('Payment attempt failed. Check your input or trying again later.')
			);
		}
	...
	public function capture($payment, $amount)
	{
		...
		try {
			...
			$genesis
				->request()
					->setTransactionId(
						Mage::helper('ecomprocessing_genesis')->genTransactionId()
					)
					->setRemoteIp(
			...
			Mage::logException($exception);
			Mage::throwException(
				Mage::helper('ecomprocessing_genesis')->__('Unsuccessful Capture transaction!')
			);
		}
	...
	public function refund($payment, $amount)
	{
		...
		try{
			...
			Mage::logException($exception);
			Mage::throwException(
				Mage::helper('ecomprocessing_genesis')->__('Unsuccessful Refund transaction!')
			);
		}
	...
	public function void($payment)
	{
		...
		try{
			...
			Mage::logException($exception);
			Mage::throwException(Mage::helper('ecomprocessing_genesis')->__('Unsuccessful Void transaction!'));
		}
